...
namespace DrRobot.JaguarControl
{
    public partial class JaguarCtrl : Form
    {
        ...
        private static int constXCenter = xMin + paneWidth *2/ 6;//changed this
        private static int constYCenter = yMin + paneHeight *2/ 6;//changed this
       ...
       // private static Pen wtrajPen = new Pen(Color.White, 2);
        ...
        private void Animate()
        {
            ...
            {
                int xCenter, yCenter;
                int gridShiftX = 0, gridShiftY = 0;
                if (followRobot.Checked)
                {
                    xCenter = (int)(constXCenter - navigation.x * mapResolution);
                    yCenter = (int)(constYCenter + navigation.y * mapResolution);
                    gridShiftX = (int)(navigation.x * mapResolution / cellWidth);
                    gridShiftY = (int)(navigation.y * mapResolution / cellWidth);
                    
                }
                else
                {
                    xCenter = constXCenter;
                    yCenter = constYCenter;
                ...
                // Add Grid <- Find the optimal number to cover the entire map
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            ...
            navigation.initialX = double.Parse(iniXBox.Text);
            navigation.initialY = double.Parse(iniYBox.Text);
            navigation.initialT = double.Parse(iniTBox.Text);
            navigation.logLaserData = logLaserData.Checked;
            navigation.logParticles = logParticles.Checked;
        ...
        private void iniXBox_TextChanged(object sender, EventArgs e)
        {
        }

        private void txtNumParticles_TextChanged(object sender, EventArgs e)
        {

        }

        private void logLaserData_CheckedChanged(object sender, EventArgs e)
        {
            navigation.logLaserData = logLaserData.Checked;
        }

        private void logParticles_CheckedChanged(object sender, EventArgs e)
        {
            navigation.logParticles = logParticles.Checked;
        }

        private void logParticleEst_CheckedChanged(object sender, EventArgs e)
        {
            navigation.logParticleEst = logParticleEst.Checked;
        }

        private void trackTrajPD_Click(object sender, EventArgs e)
        {
            navigation.trackTrajPD = !navigation.trackTrajPD;
        }

        private void chkTrackTraj_CheckedChanged(object sender, EventArgs e)
        {
            navigation.trackTrajPD = chkTrackTraj.Checked;
        }
